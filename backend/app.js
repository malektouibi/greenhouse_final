...
app.get('/api/greenhouse/:greenhouse_id', async (req, res) => {
    try {
        const greenhouseEntry = await Greenhouse.findOne({ _id: req.params.greenhouse_id });
...
app.delete('/api/greenhouse/:greenhouse_id', async (req, res) => {
    try {
        const removed_GrnEntry = await Greenhouse.remove({ _id: req.params.greenhouse_id });
